...
1. **面级特征提取**：
   ...
   * **法向代理 (Normal Proxy)**：对解析平面优先读取 OpenCASCADE 有向解析法向；圆柱面轴线作为独立的 `axis_proxy` 保存，不作为曲面法向；非平面面片优先从有效曲面采样网格估计局部法向，失败后依次退化为 PCA 法向和包围盒短轴法向。
...
在构建完包含完整物理特征和支撑细节的 $M_{raw}$ 后，首先通过紧凑化函数 $C$ 对完整弱结构证据图 $M_{raw}$ 进行节点预算、关系筛选与支撑压缩，得到紧凑结构基元图 $M_c$；随后通过蒸馏函数 $D$ 去除面组级支撑节点和局部拓扑关系，得到面向生成模型的稀疏结构先验 $S$。
