    ...
    for (i, j), types in pair_types.items():
        if "adjacent_to" in types:
            ...
            if type_i == 0 and type_j == 0 and "coplanar_with" in types:
                uf.union(i, j)
            # 圆柱面之间：合并相邻分瓣面，但必须满足轴线平行且半径一致的严格几何约束
                ...
                if norm_i > 1e-5 and norm_j > 1e-5:
                    axis_i = axis_i / norm_i
                    axis_j = axis_j / norm_j
                    axis_parallel = abs(np.dot(axis_i, axis_j)) >= np.cos(np.radians(1.0))
                else:
                    axis_parallel = True
                ...
                if axis_parallel and radius_equal:
                    uf.union(i, j)
...
def _spacing_regular_score(centroids: np.ndarray) -> Dict[str, Any]:
    N = centroids.shape[0]
    if N <= 1:
        return {
...
def _make_prior_stats(graph: Dict[str, Any], nodes: Sequence[Dict[str, Any]], relations: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "prior_compression_ratio": compression,  # 兼容旧代码，保留该键指向保留比例
        "prior_node_retention_ratio": float(len(nodes) / raw_nodes),
        "prior_node_compression_ratio": float(len(nodes) / raw_nodes),  # 兼容旧代码
        "prior_relation_retention_ratio": float(len(relations) / raw_relations),
        "prior_relation_compression_ratio": float(len(relations) / raw_relations),  # 兼容旧代码
...
def build_motif_graph(data: Dict[str, Any], raw_mode: bool = False) -> Dict[str, Any]:
    ...
    while head < len(queue):
        ...
        for neighbor in side_adj.get(curr, []):
            # 允许 BFS 跨越侧面圆角以连通整个外侧壁，由于顶底板已在 plate_faces 中排除，BFS 绝不可能进入孤立的内孔
    ...
    for group_node in face_group_nodes:
        ...
        for fid in group_node["face_ids"]:
            if fid < len(face_types) and face_types[fid] == 1:
                p = np.asarray(face_features[fid]["centroid"], dtype=np.float32)
                n = np.asarray(face_features[fid]["normal_proxy"], dtype=np.float32)
...
def extract_motif_graphs(workdir: str, parsed_dir: str | None = None, manifest_path: str | None = None) -> Dict[str, Any]:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        try:
            ...
            # 4. S (用于神经网络生成的稀疏先验图)
            prior_graph = make_motif_prior_graph(motif_compact)
