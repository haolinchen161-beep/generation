    ...
    for (i, j), types in pair_types.items():
        if "adjacent_to" in types:
            ...
            if type_i == 0 and type_j == 0 and "coplanar_with" in types:
                uf.union(i, j)
            # 圆柱面之间：通过相邻合并（如拼接多瓣圆柱面、上下半孔/半圆角）
...
def _make_prior_stats(graph: Dict[str, Any], nodes: Sequence[Dict[str, Any]], relations: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    raw_nodes = max(len(graph.get("motif_nodes", [])), 1)
    raw_relations = max(len(graph.get("motif_relations", [])), 1)
    ...
    compression = float((len(nodes) + len(relations)) / max(raw_nodes + raw_relations, 1))
    ...
    return {
        ...
        "prior_node_compression_ratio": float(len(nodes) / raw_nodes),
        "prior_relation_compression_ratio": float(len(relations) / raw_relations),
...
def make_motif_prior_graph(graph: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for rel in raw_relations:
        ...
        src = str(rel.get("source", ""))
        dst = str(rel.get("target", ""))
        src_candidates = {src} if src in selected_raw_ids else set(support_to_prior.get(src, set()))
        dst_candidates = {dst} if dst in selected_raw_ids else set(support_to_prior.get(dst, set()))
        for src_id in sorted(src_candidates):
            for dst_id in sorted(dst_candidates):
                _add_prior_relation(relation_records, src_id, dst_id, rel, rel)
    ...
    prior_graph["prior_definition"] = "S = D(M_raw)"
    prior_graph["source_graph_view"] = graph.get("graph_view", "M_raw")
...
def build_motif_graph(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 动态确定顶底平板及外部侧边面片组（用于精确区分圆孔与圆角）
    sorted_faces_by_area = sorted(range(len(face_features)), key=lambda idx: float(face_features[idx].get("area_proxy", 0.0)), reverse=True)
    plate_faces = set()
    thick_axis = 1  # 默认Y是厚度方向
    if len(sorted_faces_by_area) >= 2:
        plate1, plate2 = sorted_faces_by_area[0], sorted_faces_by_area[1]
        p1_area = float(face_features[plate1].get("relative_area", 0.0))
        p2_area = float(face_features[plate2].get("relative_area", 0.0))
        if p1_area > 0.4 and p2_area > 0.4:
            plate_faces.add(plate1)
            plate_faces.add(plate2)
            normal1 = face_features[plate1].get("normal_proxy", [0.0, 1.0, 0.0])
            normal1_abs = [abs(x) for x in normal1]
    ...
    max_loop_groups = max(4, min(16, int(0.25 * face_count_for_caps) + 1))
    ...
    for group_node in face_group_nodes:
        ...
        # 排除已经是孔特征的候选，并且要求过渡圆角必须邻接至少一个非大平板的侧边平面
        ...
        if (
            ...
            and (high_aspect or curvature_flag or ortho_support > 0)
...
def extract_motif_graphs(workdir: str, parsed_dir: str | None = None, manifest_path: str | None = None) -> Dict[str, Any]:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        try:
            ...
            structural_graph = make_structural_graph(graph)
            prior_graph = make_motif_prior_graph(graph)
