...
def _is_prior_relation_compatible(rel_type: str, src_type: str, dst_type: str) -> bool:
    ...
    if rel_type in SYMMETRIC_PRIOR_RELATIONS:
        # 平行、共面、正交、相对关系仅限在两个主要板系统 (sheet_like_group) 之间建立
        return _unordered_type_pair(src_type, dst_type) == ("sheet_like_group", "sheet_like_group")

    if rel_type == "bounded_by":
        # 有界关系仅限在局部开口/圆角特征与主体主板/边界角色之间建立（单向：局部基元 bounded_by 宿主）
        return src_type in {"loop_or_hole", "transition_group"} and dst_type in {"sheet_like_group", "boundary_group"}
...
def make_motif_prior_graph(graph: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for (src_id, dst_id, rel_type), witnesses in relation_buckets.items():
        ...
        # 计算底层面组见证覆盖率（由于高层主板节点通常有一个主支撑面组，这里用见证数来逼近覆盖率）
        support_ratio = min(1.0, float(len(unique_support_pairs)))
