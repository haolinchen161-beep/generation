...
def _aggregate_prior_graphs(prior_graphs: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    ratio_Mraw_to_Mc_values: List[float] = []
    ratio_Mc_to_S_values: List[float] = []
    ratio_Mraw_to_S_values: List[float] = []
    ready_count = 0
    ...
    for graph in prior_graphs:
        ...
        ratio_Mraw_to_Mc_values.append(float(stats.get("ratio_Mraw_to_Mc", 0.0)))
        ratio_Mc_to_S_values.append(float(stats.get("ratio_Mc_to_S", 0.0)))
        ratio_Mraw_to_S_values.append(float(stats.get("ratio_Mraw_to_S", 0.0)))
        ...
        rows.append(
            {
                ...
                "ratio_Mraw_to_Mc": stats.get("ratio_Mraw_to_Mc", 0.0),
                "ratio_Mc_to_S": stats.get("ratio_Mc_to_S", 0.0),
                "ratio_Mraw_to_S": stats.get("ratio_Mraw_to_S", 0.0),
    ...
    return {
        ...
        "ratio_Mraw_to_Mc_stats": summarize_numeric(ratio_Mraw_to_Mc_values),
        "ratio_Mc_to_S_stats": summarize_numeric(ratio_Mc_to_S_values),
        "ratio_Mraw_to_S_stats": summarize_numeric(ratio_Mraw_to_S_values),
...
def write_motif_reports(
    ...
    extraction_report = [
        ...
        f"M_raw -> M_c 压缩保留率 最小/均值/最大：{prior_aggregate.get('ratio_Mraw_to_Mc_stats')}",
        f"M_c -> S 蒸馏保留率 最小/均值/最大：{prior_aggregate.get('ratio_Mc_to_S_stats')}",
        f"M_raw -> S 总保留率 最小/均值/最大：{prior_aggregate.get('ratio_Mraw_to_S_stats')}",
