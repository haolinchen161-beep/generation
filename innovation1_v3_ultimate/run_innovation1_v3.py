...
def extract_motif(args: argparse.Namespace) -> Dict[str, Any]:
    if args.limit > 0:
        ...
        manifest_path = os.path.join(dirs["parsed"], "clean_manifest.csv")
        temp_manifest = os.path.join(dirs["parsed"], "clean_manifest_temp_limit.csv")
        # Load, slice, and save temporary manifest with limit
        import pandas as pd
        df = pd.read_csv(manifest_path, encoding='utf-8-sig')
        df.head(args.limit).to_csv(temp_manifest, index=False, encoding='utf-8-sig')
        print(f"Limiting motif extraction to first {args.limit} files using temporary manifest.")
        result = extract_motif_graphs(args.workdir, manifest_path=temp_manifest)
    else:
