...
def extract_motif_features(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for i, j in combinations(range(face_count), 2):
        ...
        if src_src in {"analytical_plane", "analytical_surface_axis"} and dst_src in {"analytical_plane", "analytical_surface_axis"}:
            relation_evidence_quality = "analytical"
        elif src_src in {"analytical_plane", "analytical_surface_axis"} or dst_src in {"analytical_plane", "analytical_surface_axis"}:
        ...
        if face_adj[i, j] > 0:
            ...
            if b_norms_i and b_norms_j:
                ...
                for ni in b_norms_i:
                    for nj in b_norms_j:
                        dot_val = abs(float(np.dot(ni, nj)))
