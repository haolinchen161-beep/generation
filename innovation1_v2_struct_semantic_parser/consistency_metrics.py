...
def _load_parse_manifest(path: str) -> Dict[str, Dict[str, str]]:
    if not os.path.exists(path):
        return {}
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        return {str(row.get("uid", "")): row for row in csv.DictReader(f)}
...
def _core_mechanism_acc(proc_mechs: Sequence[str], inf_mechs: Sequence[str]) -> float:
    a = set(proc_mechs) & CORE_TOPOLOGY_MECHANISMS
    b = set(inf_mechs) & CORE_TOPOLOGY_MECHANISMS
    if not a and not b:
        return 1.0
    return float(len(a & b) / max(len(a | b), 1))
...
def _average_rows(rows: Sequence[Dict[str, Any]], part_type: str) -> Dict[str, Any]:
    ...
    abs_vals = [float(r["parameter_l1_abs_mm"]) for r in rows if str(r.get("parameter_l1_abs_mm", "")) != ""]
    out["parameter_l1_abs_mm"] = round(mean(abs_vals), 5) if abs_vals else ""
    out["parameter_l1_abs_mm_available_count"] = len(abs_vals)
    out["parameter_l1_abs_mm_available_rate"] = round(len(abs_vals) / max(len(rows), 1), 5) if rows else 0.0
...
def evaluate_consistency(workdir: str) -> Dict[str, Any]:
    ...
    parse_manifest = _load_parse_manifest(os.path.join(parsed_dir, "parse_manifest.csv"))
    ...
    for uid in uids:
        ...
        parse_info = parse_manifest.get(uid, {})
        sampling_quality = str(parse_info.get("geometry_sampling_quality", "unknown"))
        parser_backend = str(parse_info.get("parser_backend", "unknown"))
        abs_mm_available = sampling_quality == "bbox_fallback_sampling"
        ...
        row = {
            ...
            "geometry_sampling_quality": sampling_quality,
            ...
            "parameter_l1_abs_mm_available": int(abs_mm_available),
            "parameter_l1_abs_mm": _parameter_l1_abs_mm(proc_params, inf_params, str(jd.get("part_type", "unknown"))) if abs_mm_available else "",
            ...
            "core_topology_mechanism_acc": _core_mechanism_acc(proc_mechs, inf_mechs),
        }
    ...
    report = [
        ...
        "  parameter_l1_abs_mm is emitted only when geometry_sampling_quality=bbox_fallback_sampling; otherwise it is left blank.",
        "  core_topology_mechanism_acc compares only coarse mechanisms and excludes fine fillet-transition tags.",
        "",
        "Summary by part_type:",
    ]
    for item in summary_rows:
        abs_mm_text = f"{float(item['parameter_l1_abs_mm']):.4f}" if str(item.get("parameter_l1_abs_mm", "")) != "" else "NA"
        line = (
            ...
            "param_l1_scale={parameter_l1_scale_normalized:.4f}, param_l1_abs_mm={abs_mm_text}, "
            "abs_mm_available={parameter_l1_abs_mm_available_count}/{sample_count}, "
            "hole_acc={hole_detection_acc:.4f}, stiffener_acc={stiffener_count_acc:.4f}, "
            "mech_acc={topology_mechanism_acc:.4f}, core_mech_acc={core_topology_mechanism_acc:.4f}"
        )
        report.append(line.format(**item, abs_mm_text=abs_mm_text))
