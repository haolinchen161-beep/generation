...
- 矩形圆角开口采用局部规则区分大面积 panel、直线 cutout wall 和小半径 corner transition；外侧厚度边界面标记为 boundary，但构型图关系只保留主要 start/end boundary，避免把板外轮廓误认为 stiffener 或 cutout 结构。
