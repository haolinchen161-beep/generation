...
def inspect_source(source_dir: str, workdir: str) -> Dict[str, Any]:
    ...
    report.extend(
        [
            ...
            "  - tensor_schema/data_splits/face_group_index output style for innovation2 training",
            "",
            "Keyword evidence:",
        ]
    )
...
def audit_all(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if all_summary:
        report.extend(
            [
                f"  - face_role_acc: {all_summary.get('face_role_acc', 0.0)}",
                f"  - face_group_iou_mean: {all_summary.get('face_group_iou_mean', 0.0)}",
                f"  - assign_ratio: {all_summary.get('assign_ratio', 0.0)}",
                f"  - node_type_acc: {all_summary.get('node_type_acc', 0.0)}",
                f"  - relation_type_acc: {all_summary.get('relation_type_acc', 0.0)}",
    ...
    report.extend(
        [
            ...
            "  - Use enhanced_parsed/tensor_schema.json, data_splits.csv and face_group_index.jsonl as the training interface.",
