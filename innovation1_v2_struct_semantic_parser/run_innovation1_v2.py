...
def audit_all(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if all_summary:
        abs_mm_value = all_summary.get("parameter_l1_abs_mm", "")
        abs_mm_text = abs_mm_value if str(abs_mm_value) != "" else "NA"
        report.extend(
            [
                ...
                f"  - parameter_l1_scale_normalized: {all_summary.get('parameter_l1_scale_normalized', 0.0)}",
                f"  - parameter_l1_abs_mm: {abs_mm_text}",
                f"  - parameter_l1_abs_mm_available_count: {all_summary.get('parameter_l1_abs_mm_available_count', 0)}",
                f"  - topology_mechanism_acc: {all_summary.get('topology_mechanism_acc', 0.0)}",
                f"  - core_topology_mechanism_acc: {all_summary.get('core_topology_mechanism_acc', 0.0)}",
            ]
        )
    report.extend(
        [
            ...
            "  - Curved panels use circular-arc swept solids; tapered C/hat use lofted sections and runout ribs use toe-height transition sections.",
            "  - Runout ribs use a non-degenerate toe-height transition approximation, not exact zero-height disappearance.",
