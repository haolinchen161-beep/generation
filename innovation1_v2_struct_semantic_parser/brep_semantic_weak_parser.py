...
def _ensure_minimal_fields(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "geometry_sampling_quality" not in data:
        backend = str(data.get("parser_backend", "unknown"))
        data["geometry_sampling_quality"] = "true_or_dtg_sampling" if backend == "dtg_occwl" else "bbox_fallback_sampling"
    if "surface_metadata_order_verified" not in data:
        data["surface_metadata_order_verified"] = str(data.get("parser_backend", "")) != "dtg_occwl"
    return data
...
def _weak_align_face_groups(json_data: Dict[str, Any], pkl_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    features = extract_semantic_features(pkl_data)
    part_type = str(json_data.get("part_type", ""))
    roles = _classify_faces(features, part_type)
    weak_groups = _group_faces(roles, features, part_type)
        ...
        aligned.append(
            {
                ...
                "assignment_status": "weak_aligned_by_v2_geometry_rule" if face_ids else "empty_or_not_detected",
            }
        )
    return aligned
...
def parse_enhanced_dataset(workdir: str, seed: int = 42) -> Dict[str, Any]:
    ...
    for uid in uids:
        ...
        try:
            ...
            aligned_groups = _weak_align_face_groups(jd, data)
            jd["weak_aligned_face_groups"] = aligned_groups
            jd["weak_aligned_face_group_note"] = (
                "Face ids are weakly aligned during parse_enhanced using geometry rules. "
                "They are not procedural/manual face-level ground truth."
            )
            ...
            face_group_records.append(
                {
                    "uid": uid,
                    "part_type": part_type,
                    "label_source": "weak_geometry_alignment",
                    "is_manual_ground_truth": False,
                    "is_procedural_face_truth": False,
                    "face_groups": aligned_groups,
                    "weak_aligned_face_groups": aligned_groups,
                }
            )
            ...
            records.append(
                {
                    ...
                    "surface_metadata_order_verified": bool(data.get("surface_metadata_order_verified", False)),
                    "face_count": face_count,
                    "edge_count": edge_count,
                    "vertex_count": vertex_count,
            ...
            records.append(
                {
                    ...
                    "surface_metadata_order_verified": False,
                    "face_count": 0,
                    "edge_count": 0,
                    "vertex_count": 0,
    ...
    write_jsonl(os.path.join(parsed_dir, "weak_aligned_face_group_index.jsonl"), face_group_records)
    ...
    write_csv(
        ...
        [
            ...
            "surface_metadata_order_verified",
            "face_count",
            "edge_count",
            "vertex_count",
    ...
    write_jsonl(os.path.join(parsed_dir, "weak_aligned_face_group_index_lite.jsonl"), lite_face_group_records)
    ...
    sampling_quality_counts: Dict[str, int] = {}
    for item in good:
        key = str(item.get("geometry_sampling_quality", "unknown"))
        sampling_quality_counts[key] = sampling_quality_counts.get(key, 0) + 1
    surface_order_verified = sum(1 for item in good if bool(item.get("surface_metadata_order_verified", False)))
    report = [
        ...
        "",
        "Geometry sampling quality:",
        f"  - true_or_dtg_sampling: {sampling_quality_counts.get('true_or_dtg_sampling', 0)}",
        f"  - bbox_fallback_sampling: {sampling_quality_counts.get('bbox_fallback_sampling', 0)}",
        f"  - surface metadata order verified: {surface_order_verified}",
        f"  - surface metadata order unverified: {len(good) - surface_order_verified}",
        "",
    ...
    report.extend(
        [
            ...
            "  For innovation2 WCS geometry losses, prefer true_or_dtg_sampling samples; bbox_fallback_sampling is for bbox/topology/semantic audit only.",
            "  weak_aligned_face_group_index*.jsonl records weak face alignment, not manual/procedural face-level truth.",
