...
def _classify_faces(features: Dict[str, Any], part_type: str = "") -> Dict[int, str]:
    ...
    for face in faces:
        ...
        if "curved" in part_type:
            non_flat_y = dy >= max(0.08 * gd[1], thin_floor)
            large_curved_panel_like = wide_x and long_z and area >= max(small_area, 0.45 * large_area)
            if large_curved_panel_like and not end_like and (curved_face or non_flat_y or not surface_order_verified):
                roles[fid] = "panel"
                continue
...
def _estimate_parameters(groups: List[Dict[str, Any]], features: Dict[str, Any], part_type: str = "") -> Dict[str, float]:
    gd = np.asarray(features["global_dims"], dtype=float)
    ...
    if part_type in {"tapered_c_channel", "tapered_hat_stiffener"}:
        ...
        if part_type == "tapered_hat_stiffener":
            ...
            params["flange_width_start"] = round(float(start_total), 3)
            params["flange_width_end"] = round(float(end_total), 3)
...
def infer_sample_semantics(json_data: Dict[str, Any], pkl_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    params = _estimate_parameters(groups, features, part_type)
