...
def _classify_faces(features: Dict[str, Any], part_type: str = "") -> Dict[int, str]:
    ...
    for face in faces:
        ...
        if "curved" in part_type:
            ...
            if large_panel_like and not end_like:
                roles[fid] = "panel"
                continue
...
def _infer_relations(groups: List[Dict[str, Any]], features: Dict[str, Any], part_type: str = "") -> Tuple[List[Dict[str, str]], int]:
    ...
    for group in groups:
        if group["role"] in {"hole", "cutout", "boundary"}:
            target = "panel_0" if has_panel else web_groups[0]["node_id"] if web_groups else None
...
def _estimate_parameters(groups: List[Dict[str, Any]], features: Dict[str, Any], part_type: str = "") -> Dict[str, float]:
    ...
    if beam_like:
        ...
        params["thickness"] = round(float(np.median(thin_candidates)), 3) if thin_candidates else round(float(min(gd)), 3)
    else:
