...
def _classify_faces(features: Dict[str, Any], part_type: str = "") -> Dict[int, str]:
    ...
    for face in faces:
        ...
        if part_type == "panel_with_rectangular_cutout":
            opening_thin = dy <= max(0.65 * gd[1], thin_floor)
            corner_like = (
                dx > abs_floor
                and dz > abs_floor
                and dx <= max(0.08 * gd[0], thin_floor)
                and dz <= max(0.08 * gd[2], thin_floor)
            )
            wall_like = (
                (dx <= max(0.04 * gd[0], thin_floor) and dz <= 0.45 * gd[2])
                or (dz <= max(0.04 * gd[2], thin_floor) and dx <= 0.95 * gd[0])
                or internal_xz
            )
            if opening_thin and not end_like and corner_like:
                roles[fid] = "transition"
                continue
            if opening_thin and not end_like and wall_like:
                roles[fid] = "cutout"
                continue

        if part_type == "panel_with_circular_cutout":
            opening_through_thickness = dy <= max(1.10 * gd[1], thin_floor)
            compact_opening = dx <= 0.45 * gd[0] and dz <= 0.45 * gd[2]
            if internal_xz and opening_through_thickness and compact_opening:
                roles[fid] = "hole"
                continue
        ...
        if part_type in plain_panel_types and near_x_side and long_z and dx <= max(0.04 * gd[0], thin_floor):
            roles[fid] = "boundary"
            continue
...
def _group_faces(roles: Dict[int, str], features: Dict[str, Any], part_type: str = "") -> List[Dict[str, Any]]:
    ...
    if boundary_faces:
        z_tol = max(0.04 * float(gd[2]), 1e-6)
        start = [fid for fid in boundary_faces if face_by_id[fid]["centroid"][2] <= float(features["global_min"][2]) + z_tol]
        end = [fid for fid in boundary_faces if face_by_id[fid]["centroid"][2] >= float(features["global_max"][2]) - z_tol]
        add_group("boundary", start, "boundary_start")
        add_group("boundary", end, "boundary_end")

    if part_type == "panel_with_rectangular_cutout":
        cutout_ids = [fid for fid, role in roles.items() if role == "cutout"]
        transition_ids = [fid for fid, role in roles.items() if role == "transition"]
        add_group("cutout", cutout_ids, "cutout_0")
        add_group("transition", transition_ids, "transition_0")
    else:
    ...
    for role, axes, scale in [
        ...
        if part_type == "panel_with_rectangular_cutout" and role in {"cutout", "transition"}:
            continue
