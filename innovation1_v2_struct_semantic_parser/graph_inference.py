...
def _classify_faces(features: Dict[str, Any], part_type: str = "") -> Dict[int, str]:
    ...
    for face in faces:
        ...
        curved_face = float(face.get("curvature_proxy", 0.0)) > 0.5
...
def _group_faces(roles: Dict[int, str], features: Dict[str, Any]) -> List[Dict[str, Any]]:
    gd = np.asarray(features["global_dims"], dtype=float)
    ...
    for role, axes, scale in [
        ...
        if not ids:
            continue
        radius = max(float(np.linalg.norm(gd[list(axes)])) * scale, 6.0)
...
def _infer_relations(groups: List[Dict[str, Any]], features: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    for group in groups:
        ...
        if group["role"] in {"stiffener", "web", "flange", "runout"}:
...
def infer_sample_semantics(json_data: Dict[str, Any], pkl_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    groups = _group_faces(roles, features)
    nodes = [{"id": g["node_id"], "type": g["role"], "face_ids": g["face_ids"]} for g in groups]
    relations = _infer_relations(groups, features)
...
def infer_semantics(workdir: str) -> Dict[str, Any]:
    ...
    report.extend(
        [
            ...
            "  procedural_face_groups are not copied into the inferred result.",
        ]
    )
    if failures:
