...
def _panel_with_ribs(params: Dict[str, float], rng: random.Random, asymmetric: bool = False, runout: bool = False):
    ...
    rib_heights = [rib_height] * len(centers)
...
def _sample_parameters(part_type: str, rng: random.Random) -> Dict[str, float]:
    ...
    if part_type == "panel_with_circular_cutout":
        ...
        rib_spacing = p["width"] / (int(p["rib_count"]) + 1)
        max_r = min(60.0, 0.15 * min(p["length"], p["width"]), 0.22 * max(rib_spacing - p["rib_width"], 1.0))
        min_r = max(10.0, 4.0 * p["thickness"])
        p["hole_radius"] = ru(min_r, max(min_r, max_r), 1)
    ...
    if part_type in stiffened_height_types:
        p["height"] = round(p["thickness"] + p["rib_height"], 1)
...
def _circular_hole_positions(
    ...
    if avoid_x_centers:
        ...
        if not x_candidates:
            raise ValueError("No safe hole position avoiding stiffeners")
...
def _build_graph(part_type: str, params: Dict[str, float], mechanisms: Sequence[str]) -> Dict[str, List[Dict[str, Any]]]:
    beam_types = {"tapered_c_channel", "tapered_hat_stiffener"}
    main_node_id = "web_0" if part_type in beam_types else "panel_0"
    main_node_type = "web" if part_type in beam_types else "panel"
    nodes: List[Dict[str, Any]] = [{"id": main_node_id, "type": main_node_type}]
    ...
    if part_type in beam_types:
        ...
        if part_type == "tapered_hat_stiffener":
            ...
            add_rel("transition_web_cap_0", "cap_0", "smooth_connected")
            add_node("transition_web_cap_1", "transition")
            add_rel("transition_web_cap_1", "web_0", "smooth_connected")
            add_rel("transition_web_cap_1", "cap_0", "smooth_connected")
    ...
    for side in ["start", "end"]:
        ...
        add_rel(f"boundary_{side}", main_node_id, "boundary_of")
    ...
    if "fillet_transition" in mechanisms:
        for node in [n["id"] for n in nodes if n["type"] in {"cutout", "hole"}]:
            ...
            add_rel(tid, main_node_id, "smooth_connected")
    ...
    return [
        {
            ...
            "role": node["type"],
            "face_ids": [],
            "assignment_status": "not_directly_available_from_procedural_generator",
        }
        for node in graph.get("nodes", [])
    ]


def _expected_roles_from_graph(graph: Dict[str, List[Dict[str, Any]]]) -> Dict[str, int]:
    counts: Dict[str, int] = {}
    for node in graph.get("nodes", []):
        role = str(node.get("type", "unknown"))
...
def _build_shape(part_type: str, params: Dict[str, float], rng: random.Random):
    ...
    if "root_fillet_transition" in mechanisms or "web_flange_fillet" in mechanisms or "web_cap_fillet" in mechanisms:
        ...
        fillet_note = "no structural transition fillet required for this part type; global all-edge fillet is not used"
...
def generate_enhanced_dataset(
            ...
            for attempt in range(max_attempts):
                ...
                try:
                    ...
                    sample = {
                        ...
                        "procedural_config_graph": graph,
                        "procedural_expected_roles": _expected_roles_from_graph(graph),
                        "procedural_expected_face_groups": face_groups,
                        "procedural_face_groups": face_groups,
                        "procedural_face_groups_note": (
                            "Compatibility alias for procedural_expected_face_groups. Face ids are intentionally empty; "
                            "this is not manual face-level ground truth and not direct procedural face truth."
                        ),
                        "weak_aligned_face_groups": [],
                        ...
                        "geometry_note": (
                            ...
                            "runout ribs use non-degenerate toe-height lofted transition sections."
                        ),
                    ...
                    last_error = str(exc)
                    for path in [step_path, stl_path, json_path]:
                        try:
                    ...
                    if attempt + 1 < max_attempts:
                        continue
                    failures.append({"uid": uid, "part_type": part_type, "error": last_error})
                    records.append(
                        {
                            "uid": uid,
                            "part_type": part_type,
    ...
    report_lines.extend(
        [
            ...
            "  Tapered C/hat parts use section-level web-flange/web-cap arcs; runout ribs use non-degenerate toe-height lofted transition solids.",
