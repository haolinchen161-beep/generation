...
def _surface_metadata_from_shape(shape: Any, expected_face_count: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    face_plane_locations: List[List[float]] = []
    face_plane_offsets: List[float] = []
    face_area_centroids: List[List[float]] = []
    ...
    for face in faces[:expected_face_count]:
        ...
        try:
            ...
            try:
                system = GProp_GProps()
                brepgprop.SurfaceProperties(face_obj, system)
                area = float(system.Mass())
                gp_g = system.CentreOfMass()
            ...
            if surf_type == plane_type:
                ...
                plane_location = [gp_loc.X(), gp_loc.Y(), gp_loc.Z()]
                gp_dir = gp_pln.Position().Direction()
                normal = [gp_dir.X(), gp_dir.Y(), gp_dir.Z()]
                
                # Check orientation for normal
                normal_check = normal
    ...
    return (
        ...
        np.asarray(face_plane_locations, dtype=np.float32),
        np.asarray(face_plane_offsets, dtype=np.float32),
    ...
    return ensure_minimal_fields(
        {
            ...
            "face_plane_location": plane_locations.tolist(),
            "face_plane_offset": plane_offsets.tolist(),
            "face_area_centroid": area_centroids.tolist(),
            "face_area": face_areas.tolist(),
...
def _extract_surface_types_from_split_solid(split_solid: Any) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    for i in range(face_count):
        ...
        try:
            ...
            try:
                system = GProp_GProps()
                brepgprop.SurfaceProperties(topo_face, system)
                area = float(system.Mass())
                gp_g = system.CentreOfMass()
                area_centroid = [gp_g.X(), gp_g.Y(), gp_g.Z()]
            except Exception:
...
def _parse_step_dtg(step_path: str) -> Dict[str, Any]:
    ...
    with warnings.catch_warnings(), redirect_stderr(io.StringIO()), redirect_stdout(io.StringIO()):
        ...
        if parsed_bboxes.size > 0:
            ...
            plane_locations = [(np.asarray(loc, dtype=np.float32) * s + T).tolist() for loc in plane_locations]
            area_centroids = [(np.asarray(loc, dtype=np.float32) * s + T).tolist() for loc in area_centroids]
            # 重新计算归一化坐标系下的平面方程偏移量偏移值
            plane_offsets = [-float(np.dot(plane_locations[idx], analytical_normals[idx])) for idx in range(len(plane_locations))]
            face_areas = [float(a * (s**2)) for a in face_areas]
        ...
        data["face_plane_location"] = plane_locations
        data["face_plane_offset"] = plane_offsets
        data["face_area_centroid"] = area_centroids
        data["face_area"] = face_areas
                ...
                while queue or active:
                    ...
                    if not done:
                        ...
                        if stalled:
                            ...
                            retry_items = [
                                (idx, step_path)
                                for future, (idx, step_path, _) in active.items()
                                if future not in stalled_futures
                            ]
                            for future, (idx, step_path, _) in stalled:
                                raw_results.append(_timeout_reject_result(idx, step_path, step_root, source, timeout_sec))
                                active.pop(future, None)
                                completed_pending += 1
                            for idx, step_path in retry_items:
                                queue.append((idx, step_path))
                            active.clear()
                            _terminate_executor_workers(executor)
                            timed_out_batch = True
                            print(
                                f"[parse_abc] 检测到 {len(stalled)} 个 worker 超时并记入 rejected；"
                                f"其余 {len(retry_items)} 个在途任务已重新排队。"
                            )
                            break
