...
def check_dtg_train_compatible(
    ...
    max_edge_per_face: int | None = None,
    edge_classes: int = 5,
    max_vert_face: int | None = None,
    max_num_edge_topo: int | None = None,
    max_vert: int | None = None,
    dataset: str | None = None,
) -> Tuple[bool, str, Dict[str, Any]]:
    """按数据集配置复现 DTG 训练与推理阶段的兼容性门槛。"""
    try:
        data = ensure_minimal_fields(data)
    except Exception as exc:
        return False, f"dtg_minimal_field_error:{exc}", {}

    profile = str(dataset or data.get("source", "deepcad")).strip().lower()
    if profile == "deepcad":
        max_face = 30 if max_face is None else max_face
        max_edge_per_face = 20 if max_edge_per_face is None else max_edge_per_face
        max_vert_face = 12 if max_vert_face is None else max_vert_face
        max_num_edge_topo = 108 if max_num_edge_topo is None else max_num_edge_topo
        max_vert = 186 if max_vert is None else max_vert
    elif profile == "abc":
        max_face = 30 if max_face is None else max_face
        max_edge_per_face = 20 if max_edge_per_face is None else max_edge_per_face
        max_vert_face = 15 if max_vert_face is None else max_vert_face
        max_num_edge_topo = 180 if max_num_edge_topo is None else max_num_edge_topo
        max_vert = 121 if max_vert is None else max_vert
    else:
        max_face = 32 if max_face is None else max_face
        max_edge_per_face = 30 if max_edge_per_face is None else max_edge_per_face
        max_vert_face = 8 if max_vert_face is None else max_vert_face
        max_num_edge_topo = 96 if max_num_edge_topo is None else max_num_edge_topo
        max_vert = 146 if max_vert is None else max_vert
    ...
    edge_face_adj = np.asarray(data.get("edgeFace_adj"), dtype=np.int64)
    ...
    stats = {
        ...
        "dtg_max_num_edge_topo": max_num_edge_topo,
        "dtg_max_vert": max_vert,
        "dtg_profile": profile,
    ...
    if edge_count > int(max_num_edge_topo):
        return False, "dtg_topology_edge_count_over_limit", stats
    if vertex_count > int(max_vert):
        return False, "dtg_vertex_count_over_limit", stats
    ...
    if not np.array_equal(fef_adj, fef_adj.T) or np.any(np.diag(fef_adj) != 0) or np.any(fef_adj < 0):
        return False, "dtg_fef_adj_not_simple_symmetric", stats
    if fef_adj.size and int(np.max(fef_adj)) >= int(edge_classes):
        return False, "dtg_edge_class_over_limit", stats

    if edge_face_adj.shape != (edge_count, 2):
        return False, "dtg_edgeFace_adj_invalid", stats
    if edge_face_adj.size and (
        np.any(edge_face_adj < 0)
        or np.any(edge_face_adj >= face_count)
        or np.any(edge_face_adj[:, 0] == edge_face_adj[:, 1])
    ):
        return False, "dtg_edgeFace_adj_out_of_range_or_degenerate", stats
    if edge_vert_adj.shape != (edge_count, 2):
        return False, "dtg_edgeVert_adj_invalid", stats
    if edge_vert_adj.size and (
        np.any(edge_vert_adj < 0)
        or np.any(edge_vert_adj >= vertex_count)
        or np.any(edge_vert_adj[:, 0] == edge_vert_adj[:, 1])
    ):
        return False, "dtg_edgeVert_adj_out_of_range_or_degenerate", stats

    rebuilt_fef = build_fef_adj(build_face_edge_adj(edge_face_adj, face_count), face_count)
    if not np.array_equal(rebuilt_fef, fef_adj):
        return False, "dtg_fef_edgeFace_inconsistent", stats
