...
def compute_losses(outputs, targets, epoch, warmup_epochs=10):
    ...
    # 连续参数回归误差 (除第 8 位 rib_count 以外的 8 个维度)
    cont_indices = [0, 1, 2, 3, 4, 5, 6, 8]
    loss_param_cont = F.smooth_l1_loss(pred_param[:, cont_indices], target_param[:, cont_indices])
    
    # 离散加筋数分类误差 (第 8 位)
    pred_rib_count = outputs["pred_rib_count_logits"] # [B, 6]
    target_rib_count = target_param[:, 7].long() # 转为整数索引 [B]
    loss_rib_count = F.cross_entropy(pred_rib_count, target_rib_count)
