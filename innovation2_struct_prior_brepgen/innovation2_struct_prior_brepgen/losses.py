...
def compute_losses(outputs, targets, epoch, warmup_epochs=10):
    ...
    # 连续参数回归误差：只在当前类别真实有意义的参数上计算。
    param_mask = outputs.get("pred_parameter_mask")
    if param_mask is None:
        param_mask = class_parameter_mask_tensor(
            targets["part_type_id"],
            device=pred_param.device,
            dtype=pred_param.dtype
        )

    cont_indices = torch.tensor(CONTINUOUS_PARAMETER_INDICES, device=pred_param.device, dtype=torch.long)
    cont_mask = param_mask[:, cont_indices]
    loss_param_cont_raw = F.smooth_l1_loss(
        pred_param[:, cont_indices],
        target_param[:, cont_indices],
        reduction="none"
    )
    loss_param_cont = (loss_param_cont_raw * cont_mask).sum() / (cont_mask.sum() + 1e-8)
    
    # 离散加筋数分类误差：只对 stiffened_panel 计算，其他类别 rib_count 无意义。
    pred_rib_count = outputs["pred_rib_count_logits"] # [B, 6]
    target_rib_count = target_param[:, 7].long() # 转为整数索引 [B]
    rib_count_mask = param_mask[:, PARAMETER_INDEX["rib_count"]] > 0.5
    if rib_count_mask.any():
        loss_rib_count = F.cross_entropy(pred_rib_count[rib_count_mask], target_rib_count[rib_count_mask])
    else:
        loss_rib_count = pred_rib_count.sum() * 0.0
    
    # 联合参数损失
    loss_param = loss_param_cont + loss_rib_count
    ...
    return {
        ...
        "loss_param_cont": loss_param_cont,
        "loss_rib_count": loss_rib_count,
