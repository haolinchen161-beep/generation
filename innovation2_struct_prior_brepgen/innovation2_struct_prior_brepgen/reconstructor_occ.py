...
def repair_parameters(part_type, raw_params):
    ...
    返回:
        ...
        repair_flags: 每个参数是否发生修复的 0/1 字段
    """
    repaired = {k: raw_params.get(k, PARAMETER_DEFAULTS[k]) for k in PARAMETER_KEYS}
    repair_notes = []
    repair_flags = blank_repair_flags()

    def mark_repair(key, old_val, new_val, reason):
        repair_flags[key] = 1
        repair_notes.append(f"{key}: {old_val:.2f} -> {new_val:.2f} ({reason})")
    ...
    def check_and_clip(key, min_val, max_val):
        ...
        if abs(clipped - val) > 1e-3:
            ...
            mark_repair(key, val, clipped, "clipped")

    def force_value(key, value, reason):
        val = repaired.get(key, 0.0)
        if abs(float(value) - float(val)) > 1e-3:
            repaired[key] = value
            mark_repair(key, val, value, reason)

    # 1. 只对当前类别真实有意义的连续参数执行边界硬性裁剪
    meaningful_keys = CLASS_PARAMETER_KEYS[part_type]
    for key, (min_val, max_val) in PARAMETER_BOUNDS.items():
        if key in meaningful_keys:
            check_and_clip(key, min_val, max_val)
    ...
    if abs(repaired_rc - r_count) > 1e-3:
        ...
        mark_repair("rib_count", r_count, repaired_rc, "discrete/class constraint")

    # 2. 类别无意义参数显式置零；Z-beam 的 width 按模板逻辑等于 thickness。
    if part_type == "z_beam":
        force_value("width", repaired["thickness"], "class template width=thickness")
    elif "width" not in meaningful_keys:
        force_value("width", 0.0, "class mask")

    for key in ["height", "flange_width", "rib_width", "rib_height"]:
        if key not in meaningful_keys:
            force_value(key, 0.0, "class mask")

    # 3. 关系几何约束修复 (圆角半径与厚度、高度、翻边宽度的关联约束)
    ...
    max_r = max(min_r + 0.1, max_r)
    r_clipped = np.clip(r, min_r, max_r)
    if abs(r_clipped - r) > 1e-3:
        repaired["fillet_radius"] = float(round(float(r_clipped), 2))
        mark_repair("fillet_radius", r, r_clipped, "geometric consistency")

    num_repairs = sum(repair_flags.values())
    return repaired, num_repairs, "; ".join(repair_notes), repair_flags
