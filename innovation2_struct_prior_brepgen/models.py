...
from parameter_schema import apply_class_mask_to_parameter_tensors
...
class BoundedParameterDecoder(nn.Module):
    ...
    def forward(self, z_c, class_ids=None):
        B = z_c.shape[0]
        ...
        # 动态圆角工程边界 fillet_radius，按类别选取真实相关尺寸。
        lower_bound = 1.5 * thickness_phys
        if class_ids is not None:
            if torch.is_tensor(class_ids):
                ids_tensor = class_ids.to(device=width_phys.device).view(-1)
            else:
                ids = list(class_ids) if isinstance(class_ids, (list, tuple)) else [class_ids]
                ids_tensor = torch.tensor(ids, device=width_phys.device, dtype=torch.long)
            default_min_dim = torch.minimum(
                torch.minimum(width_phys, height_phys),
                flange_width_phys
            )
            min_l_angle = torch.minimum(width_phys, height_phys)
            min_channel_or_z = torch.minimum(flange_width_phys, height_phys)
            min_hat = torch.minimum(torch.minimum(width_phys, height_phys), flange_width_phys)
            min_panel = torch.minimum(width_phys, rib_width_phys)
            min_dim = default_min_dim
            min_dim = torch.where(ids_tensor == 0, min_l_angle, min_dim)
            min_dim = torch.where((ids_tensor == 1) | (ids_tensor == 2), min_channel_or_z, min_dim)
            min_dim = torch.where(ids_tensor == 3, min_hat, min_dim)
            min_dim = torch.where(ids_tensor == 4, min_panel, min_dim)
        else:
        ...
        parameter_mask = None
        if class_ids is not None:
            pred_param_phys, pred_param_norm, parameter_mask = apply_class_mask_to_parameter_tensors(
                pred_param_phys,
                class_ids
            )
        ...
        return {
            ...
            "pred_parameter_mask": parameter_mask
        }
...
class StructuralPriorDecoder(nn.Module):
    ...
    def forward(self, z_c, class_ids=None):
        B = z_c.shape[0]
        bounded_res = self.bounded_param_decoder(z_c, class_ids=class_ids)
        ...
        return {
            ...
            "pred_parameter_mask": bounded_res["pred_parameter_mask"],
...
class StructPriorBRepCVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        struct_res = self.struct_prior_decoder(z_c, class_ids=c)
