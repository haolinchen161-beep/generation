...
from parameter_schema import (
    PARAMETER_KEYS,
    PART_TYPE_VOCAB,
    apply_class_mask_to_parameter_dict,
    meaningful_fusion_keys,
)
...
def main():
    ...
    parser.add_argument("--num_prior_samples", type=int, default=50, help="evaluate_prior 每个类别的先验生成样本数")
    ...
    part_type_vocab = dict(PART_TYPE_VOCAB)
    ...
    # ----------------------------------------------------
        ...
        with torch.no_grad():
            for sp_name, sp_ds in [("train", train_ds), ("val", val_ds), ("test", test_ds)]:
                ...
                for sample in selected:
                    ...
                    # 计算参数与几何 L1
                    param_mask_single = outputs_single["pred_parameter_mask"][0]
                    diff_p = torch.abs(outputs_single["pred_parameter_vector"][0] - targets_single["parameter_vector"][0]) * param_mask_single
                    p_l1 = diff_param_l1 = (diff_p.sum() / (param_mask_single.sum() + 1e-8)).item()
        ...
        train_rep_lines = [
            ...
            "",
            "结尾声明：",
            "本阶段构筑的是第二创新点的弱条件结构先验 B-Rep 几何生成方法实现。最终生成输入为随机隐变量 z 与可选粗粒度结构类别 c，而不是完整 configuration_graph 或完整参数表。真实 Gc 和 P 仅作为训练监督，用于学习结构构型先验生成、主结构 face group 生成、B-Rep 拓扑生成和几何生成。当前 STEP/STL 输出采用结构约束 constructive reconstruction 实现，用于验证从 z+c 到几何文件输出的链路可执行性；最终生成有效性以 prior_generation_report.txt 和 generated_samples_manifest.csv 的回读统计为准。",
            "===================================================================="
        ]
    ...
    # ----------------------------------------------------
        ...
        for pt in part_types:
            ...
            for uid_file in selected_uids:
                ...
                # 记录原始与修复
                repaired, num_rep, notes, repair_flags = repair_parameters(pt, gt_params)
                ...
                write_json({
                    ...
                    "param_repair_flags": repair_flags,
    ...
    # ----------------------------------------------------
        ...
        for plan in gen_plan:
            ...
            for _ in range(n_samples):
                ...
                pred_param_raw = apply_class_mask_to_parameter_dict(pt, pred_param_raw)
                ...
                P_layout = apply_class_mask_to_parameter_dict(pt, P_layout)
                fusion_keys = meaningful_fusion_keys(pt)

                # 计算面布局估计与解码器输出的一致性误差 (L1)
                diffs = [abs(pred_param_raw[k] - P_layout[k]) for k in fusion_keys]
                consistency_l1 = float(np.mean(diffs)) if diffs else 0.0
                ...
                fused_params = apply_class_mask_to_parameter_dict(pt, fused_params)

                # 结构修复与修正
                repaired, num_rep, notes, repair_flags = repair_parameters(pt, fused_params)
    ...
    # ----------------------------------------------------
        ...
        repair_flags_by_class = {pt: [] for pt in part_types}
        ...
        for pt in part_types:
            pt_id = part_type_vocab[pt]
            print(f" -> 正在先验生成 [{pt}] 类 {num_prior_samples} 个样本...")
            for _ in range(num_prior_samples):
                ...
                repair_flags_by_class[pt].append(repair_flags)
        ...
        repair_stats_rows = []
        for pt in part_types:
            flag_rows = repair_flags_by_class[pt]
            rep_stats = {
                "mode": "evaluate_prior",
                "class_label": pt,
                "num_samples": num_prior_samples,
            ...
            for key in parameter_keys:
                if flag_rows:
                    rep_stats[f"repair_rate_{key}"] = round(
                        sum(int(flags.get(key, 0)) for flags in flag_rows) / float(len(flag_rows)),
                        4
                    )
                else:
                    rep_stats[f"repair_rate_{key}"] = 0.0
