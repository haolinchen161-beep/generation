# -*- coding: utf-8 -*-
"""Visualization utilities for weak B-Rep motif graphs."""
...
NODE_COLORS = {
    "face_group": "#7f8c8d",
    "sheet_like_group": "#1f77b4",
    "thin_wall_pair": "#d62728",
    "loop_or_hole": "#9467bd",
    "transition_group": "#ff7f0e",
    "repeated_feature": "#2ca02c",
    "boundary_group": "#8c564b",
}
...
RELATION_COLORS = {
    ...
    "parallel_to": "#17becf",
    "opposite_to": "#d62728",
    "orthogonal_to": "#ff7f0e",
    "coplanar_with": "#1f77b4",
    "smooth_connected": "#bcbd22",
    "embedded_in": "#6c757d",
    "repeated_with": "#2ca02c",
    "bounded_by": "#8c564b",
}
...
def _simplify_graph_for_visualization(graph: Dict[str, Any], max_nodes: int = 48, max_edges: int = 54) -> tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Keep the visual graph readable while preserving the structural story.

    The JSON graph keeps dense evidence for training/audit. The PNG should show
    the high-level motif prior, not every pairwise relation.
    """
...
def _select_overlay_relations(graph: Dict[str, Any], max_edges: int = 36) -> List[Dict[str, Any]]:
    relations = [
        rel
        for rel in graph.get("motif_relations", [])
        if str(rel.get("type")) not in {"embedded_in"}
    ]
    important = [rel for rel in relations if str(rel.get("type")) in {"repeated_with", "opposite_to", "coplanar_with", "orthogonal_to", "bounded_by", "parallel_to"}]
    adjacency = [rel for rel in relations if str(rel.get("type")) == "adjacent_to"]
    important = sorted(
        important,
        key=lambda rel: (
            RELATION_VIS_PRIORITY.get(str(rel.get("type")), 99),
    ...
    adjacency = sorted(adjacency, key=lambda rel: -float(rel.get("confidence", 0.0)))
    selected = important[:max_edges]
    if len(selected) < max_edges:
        selected += adjacency[: max_edges - len(selected)]
...
def plot_faces(parsed_data: Dict[str, Any], output_path: str) -> None:
    ...
    ax.set_title(f"{_short_uid(parsed_data.get('uid', ''))} face surfaces / bbox / ids", fontsize=10)
...
def plot_motif_overlay(parsed_data: Dict[str, Any], graph: Dict[str, Any], output_path: str) -> None:
    plt = _mpl()
    features = extract_motif_features(parsed_data)
    face_features = features.get("face_features", [])
    samples = _face_samples(parsed_data)
    ...
    overlay_relations = _select_overlay_relations(graph)
    ...
    ax.set_title(
        f"{_short_uid(graph.get('uid', ''))} geometry overlay: translucent faces + face ids + {len(overlay_relations)} motif relations",
        fontsize=9,
    )
    _set_equal_axes(ax, bboxes)
    legend_handles = []
...
def plot_motif_on_solid(parsed_data: Dict[str, Any], graph: Dict[str, Any], output_path: str) -> None:
    """Render M directly on the normalized B-Rep face samples."""
    plt = _mpl()
    from matplotlib.lines import Line2D
    ...
    key_relations = [
        rel
        for rel in _select_overlay_relations(graph, max_edges=18)
        if str(rel.get("type")) in {"parallel_to", "opposite_to", "orthogonal_to", "coplanar_with", "bounded_by", "repeated_with"}
    ]
    ...
    # First pass: translucent entity ghost.
    for face in face_features:
        fid = int(face["face_id"])
        bbox = face["bbox"]
        bboxes.append(bbox)
    ...
    # Second pass: motif-bearing faces.
    for face in face_features:
        fid = int(face["face_id"])
    ...
    _set_equal_axes(ax, bboxes)
    ax.set_title(f"{_short_uid(graph.get('uid', ''))}: M on solid", fontsize=10)
    ...
    lines = [
        "M on solid",
        f"faces: {graph.get('num_faces', 0)}",
        f"motif nodes: {len(graph.get('motif_nodes', []))}",
        f"relations: {len(graph.get('motif_relations', []))}",
        "",
        "Vm on faces:",
    ]
    ...
    if len(motif_nodes) > 12:
        lines.append(f"... +{len(motif_nodes) - 12} nodes")
    lines.extend(["", "Em on faces:"])
    ...
    if len(key_relations) > 14:
        lines.append(f"... +{len(key_relations) - 14} rels")
...
def plot_motif_graph(graph: Dict[str, Any], output_path: str) -> None:
    plt = _mpl()
    try:
    ...
    nodes, relations = _simplify_graph_for_visualization(graph)
    fig, ax = plt.subplots(figsize=(9, 7))
    if not nodes:
        ax.text(0.5, 0.5, "empty motif graph", ha="center", va="center")
        ax.axis("off")
        fig.savefig(output_path, dpi=180)
        plt.close(fig)
        return
    ...
    if nx is not None:
        ...
        for typ, color in NODE_COLORS.items():
            ...
            if nodelist:
                nx.draw_networkx_nodes(g, pos, nodelist=nodelist, node_color=color, node_size=320, alpha=0.88, ax=ax, label=typ)
    ...
    ax.set_title(
        f"{_short_uid(graph.get('uid', ''))} motif graph view: {len(nodes)}/{total_nodes} nodes, {len(relations)}/{total_edges} edges",
        fontsize=9,
    )
    ax.axis("off")
...
def _cleanup_generated_visualizations(visualization_dir: str) -> Dict[str, int]:
    """Remove stale visualization PNGs before rendering the selected subset."""
...
def visualize_motifs(workdir: str, count: int = 10, uids: Sequence[str] | None = None, ready_only: bool = True) -> Dict[str, Any]:
    ...
    index_path = ready_index if ready_only and Path(ready_index).exists() else full_index
    ...
    for graph in graphs:
        ...
        plot_motif_overlay(parsed_data, graph, overlay_path)
        plot_motif_on_solid(parsed_data, graph, solid_path)
        plot_motif_graph(graph, graph_path)
